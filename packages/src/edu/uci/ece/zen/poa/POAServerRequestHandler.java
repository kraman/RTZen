...
public class POAServerRequestHandler extends
    ...
    public void handleRequest(RequestMessage req) {
        if (ZenProperties.devDbg) System.out
                .println("POAServerRequestHandler.handleRequest: Got a request to process: "
                        + req);

        // gt the index into the Active Map
        FString objKey = req.getObjectKey();

        //System.out.println("Inside ServerRequestHandler.handleRequest and mem
        // area: " + javax.realtime.RealtimeThread.getCurrentMemoryArea());
        int index = ObjectKeyHelper.getPOAIndex(objKey);
        int genCount = ObjectKeyHelper.getPOAGeneration(objKey);

        POA poa = null;

        if (ZenProperties.devDbg) System.out.println("IOR: " + index + ","
                + genCount);
        if (ZenProperties.devDbg) System.out.println("POA: " + index + ","
    ...
    public POA findPOA(byte[] poaPath) {
        ...
        /*
         * There are two things here, one is the hash map from which the poa's
         * reference is obtained. if the POA is not there in the hash map then
         * there has to be a tree traversal from the root poa to get the poa or
         * to activate the poa. / POA poa = (POA) demuxTable.get( poaPath ,
         * poaPath.length ); // Ask the POA to handle the request and return if
         * (poa != null) { return poa; } /* The POA is not present in the map
         * hence we need to parse the poa path name and try activating the POA
         */

        /*
         * POA parent = (POA) getRoot(); StringBuffer childPOAName = new
         * StringBuffer(); // The first POA name should be that of Root POA plus
         * a slash // hence we need not get that once more int index = 8; // To
         * ease the parsing of the Path Name, we add the end condition "/" // to
         * the string. poaName = poaName.concat("/"); int next =
         * Util.getPOAName(poaName, index, childPOAName); while (next != -1) {
         * parent = parent.getChildPOA(new String(childPOAName)); // go one
         * beyond the separator that is the starting // index of the next POA
         * Name index = next; childPOAName = null; // enable gc childPOAName =
         * new StringBuffer(); next = Util.getPOAName(poaName, index,
         * childPOAName); } // check if we have parsed the entire key if (index ==
         * poaName.length()) { // Add it in the POA HashMap this.addPOA(poaName,
         * parent); return parent; } else { // Logger.debug("Error: POAPathName
         * corrupted: Last POA Name parsed = " // + childPOAName + "POAPathName = " +
         * poaName); throw new org.omg.CORBA.BAD_CONTEXT("Invalid POA Path Name
         * in the ObjectKey", 0, CompletionStatus.COMPLETED_NO); }
         */
        return null;
    }
     ...
     * shutdown.
     * 
     * @param waitForRequestsToComplete
     *            if true wait for requests to complete, else shutdown now
     */
    ...
    public void remove(edu.uci.ece.zen.poa.POA poa) {
        /*
         * POAMap.remove(poa.path_name()); // remove from the Active Demux
         * Table! this.demuxTable.unbind(poa.path_name());
         */
    }

    /**
     * The ORB uses this method to check if the POA is present in the POA
     * hierarchy. The method returns true id such a POA is present or tries to
     * activate the POA. A return value of true indicates that the POA is ready
     * to service requests and the Client may safely invoke requests on that
     * POA. This interaction is similar to the request Processing mechanism in
     * the <code> handle </code> method.
     * 
     * @param poaName
     * @return boolean
     */
    ...
    public boolean processorPresent(String poaName) {
        /*
         * POA poa = (POA) POAMap.get(poaName); // Ask the POA to handle the
         * request and return if (poa != null) { return true; } //
         * Logger.debug("poaPresent:ServerRequestHandler: POA not found in the
         * Map!"); /* The POA is not present in the map hence we need to parse
         * the poa path name and try activating the POA / POA parent = (POA)
         * this.getRoot(); StringBuffer childPOAName = new StringBuffer(); //
         * The first POA name should be that of Root POA plus a slash // hence
         * we need not get that once more int index = 8; // To ease the parsing
         * of the Path Name, we add the end condition "/" // to the string.
         * poaName = poaName.concat("/"); int next = Util.getPOAName(poaName,
         * index, childPOAName); while (next != -1) { parent =
         * parent.getChildPOA(new String(childPOAName)); // go one beyond the
         * separator that is the starting // index of the next POA Name index =
         * next; childPOAName = null; // enable gc childPOAName = new
         * StringBuffer(); next = Util.getPOAName(poaName, index, childPOAName); } //
         * check if we have parsed the entire key if (index == poaName.length()) { //
         * Add it in the POA HashMap this.addPOA(poaName, parent); return true; }
         * else { return false; }
         */
        return true;
    }
    ...
    public POA find_POA(FString okey) {
        /*
         * int index = okey.poaIndex(); int genCount = okey.poaIndexGenCount();
         * if (this.demuxTable.getGenCount(index) == genCount) { return
         * this.demuxTable.mapEntry(index).poa; } else if (okey.isPersistent()) {
