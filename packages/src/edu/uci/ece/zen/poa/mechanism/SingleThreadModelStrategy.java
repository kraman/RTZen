...
public final class SingleThreadModelStrategy extends ThreadPolicyStrategy {
   ...
   /**
    * Enter block after grabbing mutex. Since multiple POAs can share 
    * enter() synchronizes unsing the <code> 
    * SingleThreadModelStrategy.class </code>
    * @param invokeHandler request handler
    */
   public synchronized void enter(
        ...
        if (servantTable.contains(invokeHandler)) {
            synchronized (SingleThreadModelStrategy.class) {
                try {
                    // edu.uci.ece.zen.orb.Logger.debug("SingleThreadedPOA:invokehandler Shared, Thread waiting for"
