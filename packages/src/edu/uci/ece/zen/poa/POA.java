/* --------------------------------------------------------------------------*
 * $Id: POA.java,v 1.2 2004/02/25 06:12:43 nshankar Exp $
 *--------------------------------------------------------------------------*/
...
public class POA extends org.omg.CORBA.LocalObject
    ...
    protected POA(final edu.uci.ece.zen.orb.ORB orb,
   ...
   private void createImpl()
    {
    	...
    	try{
                childMemory = new LTMemory(10*1024,10*1024);
                Object poa =childMemory.newInstance(POAImpl.class);
                childMemory.setPortal(poa);
                childMemory.enter( new Runnable() {
                								public void run()
                                                                                {
                                                                                	( (POAImpl) (childMemory.getPortal()) ).setParameters(orb,poaName,poaPath,policyList,poaManager,serverRequestHandler,poaDemuxIndex,numberOfCurrentRequests,childMemory);
                                                                                }
                                                                          });
...
//        	return ( (POAImpl) (childMemory.getScope()) ).activate_object (p_servant);
    ...
    public org.omg.PortableServer.POA create_POA(
	    ...
	    child  = (POA) currentMemory.newInstance(ctor,param);
            ...
            /*currentMemory.enter( new Runnable() { public void run() {
            									 child = new edu.uci.ece.zen.poa.POA(orb, adapter_name,
                    								 this.path_name() + "/" + temp, policies, this, a_POAManager);
                                                                              	 }
                                                                              }
	   */
