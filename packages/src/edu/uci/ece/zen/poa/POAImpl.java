...
public class POAImpl
{
            ...
            POARunnable prun)
    {
        ...
        try
        {
            serverRequestHandler = (POAServerRequestHandler) ((ORBImpl) orb.orbImplRegion
    ...
    public void handleRequest(RequestMessage req, POARunnable prun)
    {
        ...
        try
        {
            ...
            //ZenProperties.logger.log("POAImpl.handled 2" + ih.value);

            // Check the state of the POAManager. Here the POA is in active state
            
            
            POAManager poaMgr = (POAManager) self.the_POAManager();
            //ZenProperties.logger.log("POAImpl.handled POA manager:" + poaMgr);
            
            ih.value = POAManager.checkPOAManagerState(poaMgr);
            if (ih.value != POARunnable.NoException)
            {
                prun.exception = ih.value;
                //retIntHolder(ih);
            ...
            //ZenProperties.logger.log("POAImpl.handled 3 " + ih.value);
            ...
            //ZenProperties.logger.log("POAImpl.handled 6 " + ih.value);
            if (ih.value != POARunnable.NoException)
            {
                prun.exception = ih.value;
                //retIntHolder(ih);
            ...
            //ZenProperties.logger.log("POAImpl.handled 7 " + ih.value);
...
//            }
            ...
            orb.freeEIR(eir);
            //ZenProperties.logger.log("POAImpl.handled 15 " + ih.value);
        } 
        catch (Exception ex)
        {
     ...
     * @throws org.omg.PortableServer.POAPackage.ServantNotActive If the servant passed in is not
     *             active.
     * @throws org.omg.PortableServer.POAPackage.WrongPolicy If the policies of the POA dont contain
     *             RETAIN and UNIQUE_ID/MYULTIPLE_ID policies.
     * @return The object reference for that particular servant.
     */
    public org.omg.CORBA.Object servant_to_reference(final Servant p_servant, MemoryArea clientMemoryArea,
                                                    POARunnable prun)
    {
        ...
        case POARunnable.ServantNotActiveException:
        {
                            ...
                            .validate(edu.uci.ece.zen.poa.mechanism.IdUniquenessStrategy.MULTIPLE_ID))
            {
                ...
                this.lifespanStrategy.create(self.poaPath, oid, self.poaDemuxIndex,
                        self.poaDemuxCount, index, genCount, okey);
                retVal = this.create_reference_with_object_key(okey, p_servant._all_interfaces(
                        self, null)[0], clientMemoryArea);
                break;
            }
        ...
        case POARunnable.NoException:
        {
            ...
            this.lifespanStrategy.create(self.poaPath, oid, self.poaDemuxIndex, self.poaDemuxCount,
                    index, count, okey);

            retVal = this.create_reference_with_object_key(okey, p_servant._all_interfaces(self,
                    null)[0], clientMemoryArea);
        }
        }
        retFString(okey);
        retFString(oid);
        ...
        if (ZenProperties.dbg)
            ZenProperties.logger.log("servant_to_reference " + retVal);
        if (ZenProperties.dbg)
    ...
    public void servant_to_id(final Servant servant, MemoryArea mem, POARunnable prun)
    {
        //        FString oid = getFString();
        //        IntHolder ih = getIntHolder();
        //
        //        this.retentionStrategy.getObjectID(servant, oid, ih);
        //        switch (ih.value)
        //        {
        //            case POARunnable.ServantNotActiveException:
        //            {
    ...
    {
        if (!(idAssignmentStrategy instanceof SystemIdStrategy && retentionStrategy instanceof RetainStrategy))
        {
            prun.exception = POARunnable.WrongPolicyException;
            return;
        }
    ...
    public void activate_object_with_id(byte[] oid, Servant servant, MemoryArea mem,
            POARunnable prun)
    {
    }
    ...
    public synchronized org.omg.CORBA.Object create_reference_with_object_key(FString ok,
            final String intf, MemoryArea clientArea)
    {
        ...
        } catch (Exception e)
        {
            ZenProperties.logger
                    .log(Logger.WARN, getClass(), "create_reference_with_object_key", e);
            return null;
        }
    }
     ...
     * @param prun
     */
    public void activate_object_with_priority(Servant servant, Short priority, MemoryArea area,
            POARunnable prun)
    {
