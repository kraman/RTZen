...
public class CDROutputStream extends org.omg.CORBA.portable.OutputStream {
    ...
    public final void write_TypeCode(org.omg.CORBA.TypeCode value) {
        ...
        try {
            ...
            case TCKind._tk_union: {
                ...
                for (int i = 0; i < memCount; i++) {
                    if ( i != def_ind ) {
                        value.member_label(i).write_value(cdr);
                    }
                    else {
                        cdr.write_octet( (byte) 0 );
                    }
    ...
    // 1. The discriminator value that sets the case for this union.  It's type is determined by the typecode's data member discriminator_type.
    // 2. The actual value of the union having whatever type is
    // appropriate for the case of the union is.
    private final void write_value_union(org.omg.CORBA.TypeCode tc, org.omg.CORBA.portable.InputStream in) {
    ...
    public static final void transcribe_union(org.omg.CORBA.TypeCode tc, org.omg.CORBA.portable.InputStream in, org.omg.CORBA.portable.OutputStream out) {
        try {
            ...
            case TCKind._tk_ushort: {
                ...
                selected_index = union_find_member_label(tc, "" + aValue);
                break;
            }
            ...
            case TCKind._tk_boolean: {
                boolean aValue = in.read_boolean();
                out.write_boolean(aValue);
                selected_index = union_find_member_label(tc, "" + aValue);
                break;
            }

            case TCKind._tk_char: {
                char aValue = in.read_char();
                out.write_char(aValue);
                selected_index = union_find_member_label(tc, "" + aValue);
                break;
            }

            case TCKind._tk_wchar: {
                char aValue = in.read_wchar();
                out.write_wchar(aValue);
                selected_index = union_find_member_label(tc, "" + aValue);
                break;
            }
            ...
            // If member_index was never set, the discriminator should be
            // set to the default case because we were skipping checking
            // defaultIndex.  If no match was found between the
            // discriminator's value and the many case labels, try to set
            // member_index to the default_index.  However, defaultIndex
            // will be -1 if there is no default member, which would leave
            // member_index still as -1.
            if (selected_index == -1) {
     ...
     * selects in a union's typecode.
     *
     * @param tc TypeCode whose member labels are going to be searched for equality with the value in sthe string discriminatorValue.
    ...
    public static final int union_find_member_label(org.omg.CORBA.TypeCode tc, String discriminatorValue)
        ...
        case TCKind._tk_ushort: {
            ...
            for (int i = 0; i < tc.member_count(); i++) {
                // For the default index, the member_label is the zero
                // octet as metioned on page 10-51 of the CORBA v2.3 spec.
                if (i == defaultIndex) {
                    continue;
                }
                short memLabelShort;
                if (kind == TCKind._tk_short) {
                    memLabelShort = tc.member_label(i).extract_short();
                }
                else {
                    memLabelShort = tc.member_label(i).extract_ushort();
                }
                if ( discShort == memLabelShort) {
                    member_index = i;
                    break;
                }
            }
        ...
        case TCKind._tk_ulong: {
            ...
            for (int i = 0; i < tc.member_count(); i++) {
                int memLabelLong;
                if (kind == TCKind._tk_long) {
                    memLabelLong = tc.member_label(i).extract_long();
                }
                else {
                    memLabelLong = tc.member_label(i).extract_ulong();
                }
                if ( discLong == memLabelLong) {
                    member_index = i;
                    break;
                }
            }
        ...
        case TCKind._tk_ulonglong: {
            ...
            for (int i = 0; i < tc.member_count(); i++) {
                long memLabelLongLong;
                if (kind == TCKind._tk_longlong) {
                    memLabelLongLong = tc.member_label(i).extract_longlong();
                }
                else {
                    memLabelLongLong = tc.member_label(i).extract_ulonglong();
                }
                if ( discLongLong == memLabelLongLong) {
                    member_index = i;
                    break;
                }
            }
        ...
        case TCKind._tk_boolean: {
            ...
            for (int i = 0; i < tc.member_count(); i++) {
                boolean memLabelBoolean;
                memLabelBoolean = tc.member_label(i).extract_boolean();
                if ( discBoolean == memLabelBoolean) {
                    member_index = i;
                    break;
                }
            }
        ...
        case TCKind._tk_char:
        case TCKind._tk_wchar: {
            long discChar = discriminatorValue.charAt(0);
            for (int i = 0; i < tc.member_count(); i++) {
                char memLabelChar;
                if (kind == TCKind._tk_char) {
                    memLabelChar = tc.member_label(i).extract_char();
                }
                else {
                    memLabelChar = tc.member_label(i).extract_wchar();
                }
                if ( discChar == memLabelChar) {
                    member_index = i;
                    break;
                }
            }
