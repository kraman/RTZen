...
public privileged aspect PIServerAspect{
    ...
    private static edu.uci.ece.zen.orb.transport.Transport transport;
    private static edu.uci.ece.zen.orb.protocol.type.RequestMessage msg;
    private static edu.uci.ece.zen.orb.protocol.type.ReplyMessage replyMsg;
    ...
    pointcut getTransport(edu.uci.ece.zen.orb.transport.Transport obj):
        (execution( edu.uci.ece.zen.orb.transport.Transport.new(..)) ) && target(obj);
    ...
    private static void iterate(int initialType, Vector serverInterceptors,
        ...
        if(type == SEND_REPLY || type == SEND_EXCEPTION || type == SEND_OTHER)
        {
            ...
            if(startPointsComplete)
            {
                ...
                ZenProperties.logger.log("Resetting count: " + count);
            }
        }
        ...
        while((forward && count < serverInterceptors.size()) || (!forward && count >= 0)) {
            try {
                switch(type) {
                    ...
                    default:
                        ZenProperties.logger.log("Server interceptor of type " + type + " not found.");
                ...
                else if(type == RECEIVE_REQUEST_SERVICE_CONTEXTS)
                {
                    ...
                    count--;
                    newException = se;
                    ZenProperties.logger.log("Need to implement -- Exception handled in PI");
...
//                                             );
                ...
                {
                    ZenProperties.logger.log("Marshalling exception");

                    needToSendException = true;

                    ((edu.uci.ece.zen.orb.protocol.giop.v1_0.ReplyMessage)replyMsg).setReplyStatus(org.omg.GIOP.ReplyStatusType_1_0._SYSTEM_EXCEPTION);
