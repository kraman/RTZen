...
public class ActiveDemuxTable{
    ...
    /**
     * Link list of free nodes. All the nodes in this link list are
     * alrwady allocated in the "data" table.
     */
    ActiveDemuxTableNode freeList;

    /**
     * This method is used to push a freed node onto the freeList. This
     * method is threadsafe.
     * @param idx The node to push.
     */
    ...
    /**
     * This method is used to retrieve an unused node from the freeList.
     ...
     * You must call <code>init( &lt;numEntries&gt; )</code> after this 
     * method to initialize the ActiveDemuxTable.
     */
     ...
     * @param numEntries The number of entries to allocate in the table.
     */
    public void init( int numEntries ){
        try{
            MemoryArea mem = MemoryArea.getMemoryArea(this);
            data = (ActiveDemuxTableNode[]) mem.newArray( ActiveDemuxTableNode.class , numEntries );
            for( int i=0;i<numEntries;i++ ){
                data[i] = (ActiveDemuxTableNode) mem.newInstance( ActiveDemuxTableNode.class );
                data[i].idx = i;
                push( data[i] );
            }
        }catch( Exception e ){
    ...
    /**
     * Binds an key to an object and returns the active demux location
     * as an integer.
     * @param key The key into the ActiveDemuxTable
     * @param data The object to associate with the key
     ...
     * @param idx The active demux location to unbind.
     */
    ...
    /**
     * Retrieve the generation count of the given 
    ...
    /**
     * Retrieve the active demux location of the provided key. This
     * is a O(n) operation.
     * @param key The key into the ActiveDemuxTable
