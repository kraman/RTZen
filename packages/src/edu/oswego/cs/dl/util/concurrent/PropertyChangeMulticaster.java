/*
 * File: PropertyChangeMulticaster.java Originally written by Doug Lea and
...
/**
 * This class is interoperable with java.beans.PropertyChangeSupport, but relies
 * on a streamlined copy-on-write scheme similar to that used in
 * CopyOnWriteArrayList. This leads to much better performance in most
 * event-intensive programs. It also adheres to clarified semantics of add and
 * remove operations.
 * <p>
 * <b>Sample usage. </b>
 * 
...
public class PropertyChangeMulticaster implements Serializable {
     ...
     * names to PropertyChangeMulticaster objects.
     * 
     * @serial
     */
    ...
    protected synchronized PropertyChangeMulticaster getChild(
            String propertyName) {
     ...
     * it will receive multiple change notifications upon any firePropertyChange
     * 
     * @param listener
    ...
    public synchronized void addPropertyChangeListenerIfAbsent(
            PropertyChangeListener listener) {
    ...
    /**
     * Remove a PropertyChangeListener from the listener list. It removes at
     * most one occurrence of the given listener. If the listener was added
     * multiple times it must be removed mulitple times. This removes a
     * PropertyChangeListener that was registered for all properties, and has no
     * effect if registered for only one or more specified properties.
     * 
     * @param listener
    ...
    /**
     * Remove a PropertyChangeListener for a specific property. Affects only the
    ...
    protected void multicast(PropertyChangeEvent evt) {
        ...
        synchronized (this) {
            ...
            if (children != null && evt.getPropertyName() != null) {
                child = (PropertyChangeMulticaster) children.get(evt
                        .getPropertyName());
            }
        }
