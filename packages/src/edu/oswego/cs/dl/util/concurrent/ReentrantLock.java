/*
 * File: ReentrantLock.java Originally written by Doug Lea and released into the
 ...
 * Who What 11Jun1998 dl Create public version 5Aug1998 dl replaced int counters
 * with longs
 */
...
public class ReentrantLock implements Sync {
    ...
    public synchronized void release() {
        if (Thread.currentThread() != owner_) { throw new Error(
                "Illegal Lock usage"); }
    ...
    /**
     * Release the lock N times. <code>release(n)</code> is equivalent in
     * effect to:
     * 
     * <pre>
     * for (int i = 0; i &lt; n; ++i)
     ...
     * 
     * @exception Error
     *                thrown if not current owner of lock or has fewer than N
     *                holds on the lock
     */
    public synchronized void release(long n) {
        if (Thread.currentThread() != owner_ || n > holds_) { throw new Error(
                "Illegal Lock usage"); }
        holds_ -= n;
        if (holds_ == 0) {
            owner_ = null;
