/*
 * File: ObservableSync.java Originally written by Doug Lea and released into
 ...
 * event-style messages to other observer objects upon invocation of Sync
 * methods. These observers typically perform monitoring, logging, or other
 * bookkeeping operations surrounding the object being managed by this Sync
 * object.
 * <p>
 * Because ObservableSync does not itself perform any synchronization control,
 * the attempt operation always succeeds. This class is typically used (via
 * LayeredSync) as a wrapper around those that do perform synchronization
 * control.
 * <p>
 * This class is based around a standard Observer design pattern. It is not hard
 * to convert this to instead use a Listener design (as seen in AWT and
 * JavaBeans), by defining associated EventObjects and forwarding them.
 * <p>[ <a
...
public class ObservableSync implements Sync {
    ...
    /**
     * Create an ObservableSync that uses the supplied argument for all
     * notifications. The argument is typically an object that is being managed
     * by this Sync object.
     */
    ...
    /**
     * Return an iterator that can be used to traverse through current set of
     * observers
     */
