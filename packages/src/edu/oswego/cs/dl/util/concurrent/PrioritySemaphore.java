/*
 * File: PrioritySemaphore.java Originally written by Doug Lea and released into
...
/**
 * A Semaphore that grants requests to threads with higher Thread priority
 * rather than lower priority when there is contention. Ordering of requests
 * with the same priority is approximately FIFO. Priorities are based on
 * Thread.getPriority. Changing the priority of an already-waiting thread does
 * NOT change its ordering. This class also does not specially deal with
 * priority inversion -- when a new high-priority thread enters while a
 * low-priority thread is currently running, their priorities are <em>not</em>
...
public class PrioritySemaphore extends QueuedSemaphore {
    ...
    protected static class PriorityWaitQueue extends WaitQueue {
        ...
        protected final FIFOSemaphore.FIFOWaitQueue[] cells_ = new FIFOSemaphore.FIFOWaitQueue[Thread.MAX_PRIORITY
        ...
        protected void insert(WaitNode w) {
            int idx = Thread.currentThread().getPriority()
                    - Thread.MIN_PRIORITY;
