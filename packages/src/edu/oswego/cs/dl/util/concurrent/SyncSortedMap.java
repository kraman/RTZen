/*
 * File: SyncSortedMap.java Originally written by Doug Lea and released into the
...
/**
 * SyncSortedMaps wrap Sync-based control around java.util.SortedMaps. They
 * support the following additional reader operations over SyncMap: comparator,
...
public class SyncSortedMap extends SyncMap implements SortedMap {
    ...
    /**
     * Create a new SyncSortedMap protecting the given map, and using the given
     * sync to control both reader and writer methods. Common, reasonable
    ...
    public SortedMap subMap(Object fromElement, Object toElement) {
        ...
        try {
            return new SyncSortedMap(baseSortedMap().subMap(fromElement,
                    toElement), rd_, wr_);
        } finally {
            afterRead(wasInterrupted);
        }
    ...
    public SortedMap headMap(Object toElement) {
        ...
        try {
            return new SyncSortedMap(baseSortedMap().headMap(toElement), rd_,
                    wr_);
        } finally {
            afterRead(wasInterrupted);
        }
    ...
    public SortedMap tailMap(Object fromElement) {
        ...
        try {
            return new SyncSortedMap(baseSortedMap().tailMap(fromElement), rd_,
                    wr_);
        } finally {
            afterRead(wasInterrupted);
        }
