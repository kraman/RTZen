/*
 * File: ProperyChangeMulticaster.java Originally written by Doug Lea and
 ...
 * This class is based on Sun JDK java.beans.VetoableChangeSupport, which is
 * copyrighted by Sun. (It shares practically no code, but for consistency, the
 * documentation was lifted and adapted here.) History: Date Who What 14Mar1999
 * dl first release
 */
 ...
 * on a streamlined copy-on-write scheme similar to that used in
 * CopyOnWriteArrayList. It also adheres to clarified semantics of add, remove,
 * and fireVetoableChange operations.
 * <p>
 * <b>Sample usage. </b>
 * 
 * <pre>
 ...
 * 
 *     // vetoable listeners:
 *     protected VetoableChangeMulticaster vetoers = new VetoableChangeMulticaster(
 *             this);
 * 
 *     // Possibly also some ordinary listeners:
 *     protected PropertyChangeMulticaster listeners = new PropertyChangeMulticaster(
 *             this);
 * 
 ...
 *         if (changePending) throw new PropertyVetoException(
 ...
 *         else if (c == null) throw new PropertyVetoException(
 *                 &quot;Cannot change color to Null&quot;);
 *         else {
 *             changePending = true;
 *             return myColor;
 *         }
...
public class VetoableChangeMulticaster implements Serializable {
    ...
    /**
     * HashMap for managing listeners for specific properties. Maps property
     * names to VetoableChangeMulticaster objects.
     * 
     * @serial
     */
    ...
    protected synchronized VetoableChangeMulticaster getChild(
            String propertyName) {
    ...
    /**
     * Add a VetoableChangeListener to the listener list. The listener is
     * registered for all properties. If the listener is added multiple times,
     * it will receive multiple change notifications upon any
     * fireVetoableChange.
     * 
     * @param listener
    ...
    /**
     * Add a PropertyChangeListener to the listener list if it is not already
     * present. The listener is registered for all properties. The operation
     * maintains Set semantics: If the listener is already registered, the
    ...
    public synchronized void addVetoableChangeListenerIfAbsent(
            VetoableChangeListener listener) {
        ...
        for (int i = 0; i < len; ++i) {
            ...
            if (listener.equals(listeners[i])) { return; } // already present --
            // throw away copy
        }
        newArray[len] = listener;
        listeners = newArray;
     ...
     * It removes at most one occurrence of the given listener. If the listener
     * was added multiple times it must be removed mulitple times. This removes
     * a VetoableChangeListener that was registered for all properties, and has
     * no effect if registered for only one or more specified properties.
     * 
     * @param listener
     ...
     * property. However, if a listener is registered both for all properties
     * and a specific property, it will receive multiple notifications upon
     ...
     * already registered. The listener will be invoked only when a call on
     * fireVetoableChange names that specific property.
     * 
     * @param propertyName
     *            The name of the property to listen on.
     * @param listener
    ...
    /**
     * Remove a VetoableChangeListener for a specific property. Affects only the
     * given property. If the listener is also registered for all properties,
    ...
    /**
     * Helper method to relay evt to all listeners. Called by all public
    ...
    protected void multicast(PropertyChangeEvent evt)
        ...
        synchronized (this) {
            ...
            if (children != null && evt.getPropertyName() != null) {
                child = (VetoableChangeMulticaster) children.get(evt
                        .getPropertyName());
            }
        }
        ...
        try {
            ...
            PropertyChangeEvent revert = new PropertyChangeEvent(evt
                    .getSource(), evt.getPropertyName(), evt.getNewValue(), evt
                    .getOldValue());
            ...
            for (int k = 0; k <= lastNotified; ++k) {
                ...
                } catch (PropertyVetoException ignore) { // Cannot veto a
                    // reversion
                }
            }
     ...
     * Notifications are sent serially (although in no particular order) to the
     * list of listeners, aborting if one throws PropertyVetoException. Upon
     * this exception, fire a new event reverting this change to all listeners
     * that have already been notified (ignoring any further vetos), suppress
     * notifications to all other listeners, and then rethrow the
     ...
     *                if a recipient wishes the property change to be rolled
     *                back.
     */
    public void fireVetoableChange(String propertyName, Object oldValue,
        ...
        if (oldValue == null || newValue == null || !oldValue.equals(newValue)) {
            multicast(new PropertyChangeEvent(source, propertyName, oldValue,
                    newValue));
        }
    ...
    public void fireVetoableChange(String propertyName, int oldValue,
        ...
        if (oldValue != newValue) {
            multicast(new PropertyChangeEvent(source, propertyName,
     ...
     * PropertyVetoException.
     * <p>
     * No event is fired if old and new are equal and non-null. equal and
     * non-null.
     * 
     * @param evt
    ...
    /**
     * Check if there are any listeners for a specific property. If propertyName
     * is null, return whether there are any listeners at all.
     * 
     * @param propertyName
     *            the property name.
