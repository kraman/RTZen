/*
 * File: CopyOnWriteArrayList.java Written by Doug Lea. Adapted from JDK1.2
 * ArrayList.java which carries the following copyright: Copyright 1997 by Sun
...
public class CopyOnWriteArrayList implements List,Cloneable,
    ...
    public Object[] toArray(Object a[]) {
        ...
        if (a.length < elementData.length) {
            a = (Object[]) java.lang.reflect.Array.newInstance(a.getClass()
                    .getComponentType(), elementData.length);
        }
    ...
    public synchronized Object set(int index, Object element) {
        ...
        boolean same = (oldValue == element || (element != null && element
                .equals(oldValue)));
    ...
    public synchronized void removeRange(int fromIndex, int toIndex) {
        ...
        if (fromIndex < 0 || fromIndex >= len || toIndex > len
                || toIndex < fromIndex) { throw new IndexOutOfBoundsException(); }
    ...
    public boolean containsAll(Collection c) {
        ...
        while (e.hasNext()) {
            if (indexOf(e.next(), elementData, len) < 0) { return false; }
        }
    ...
    protected static class COWSubList extends AbstractList {
         ...
         * those defined in AbstractList, all is well. It's not clear whether
         * this is worth cleaning up. The kinds of list operations inherited
         * from AbstractList are are already so slow on COW sublists that adding
         * a bit more space/time doesn't seem even noticeable.
         */
        ...
        protected COWSubList(CopyOnWriteArrayList list, int fromIndex,
                int toIndex) {
            l = list;
        ...
        protected void checkForComodification() {
            if (l.array_ != expectedArray) { throw new ConcurrentModificationException(); }
        }
