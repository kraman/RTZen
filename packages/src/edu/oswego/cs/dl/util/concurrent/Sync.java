...
public interface Sync
{
	...
	/**
	 * Wait at most msecs to pass; report whether passed. <p> The
	 * method has best-effort semantics: The msecs bound cannot be
	 ...
	 * possible after the specified bound. Also, timers in Java do not
	 * stop during garbage collection, so timeouts can occur just
	 * because a GC intervened. So, msecs arguments should be used in
	 * a coarse-grained manner. Further, implementations cannot always
	 * guarantee that this method will return at all without blocking
	 * indefinitely when used in unintended ways. For example,
	 * deadlocks may be encountered when called in an unintended
	 * context. <p>
