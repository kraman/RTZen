...
const int sleep_time = 1000; //The unit is microsecond
const int iteration = 30000;
const int array_size = 200; //The size of the array for low priority task
const int warm_up = 5000;
int client_priority = 0; //The default priority for low task is 0
...
int main (int argc, char *argv[])
{
    ...
    ACE_TRY_NEW_ENV
    {
        ...
        else
        {
	        ...
	        //Enable the linear priority mapping in TAO so that the priority range is increased from 0-127 to 0-32767
	            CORBA::Object_var mapping_object = orb->resolve_initial_references ("PriorityMappingManager");
	            RTCORBA::PriorityMappingManager_var mapping_manager = RTCORBA::PriorityMappingManager::_narrow (mapping_object.in () ACE_ENV_ARG_PARAMETER);  
		    ACE_TRY_CHECK;	    
	            RTCORBA::PriorityMapping *pm = new TAO_Linear_Priority_Mapping (SCHED_OTHER); 
	            mapping_manager->mapping (pm); 
            ...
            CORBA::Short server1_priority =
                check_sd_policy (server1.in () ACE_ENV_ARG_PARAMETER);
            ACE_TRY_CHECK;
