...
title('Roundtrip Latency/Jitter, constant size (128 bytes), Two Hosts on Emulab');
...
xIndex = index * difference;
	...
	fileName = strcat(fNameBase, num2str(round(index*10)),'.', num2str(round(index )),'.', num2str(round(128)), '.txt');
        s = load(fileName);
	s = diff(s(:, 3));
...
print -depsc2 latency_remote.eps



%saveas(gcf,'latency_remote','epsc2')
